...
{	
	...
	LCD_OpenWindow ( usCOLUMN, usPAGE, dotsize, dotsize );
	LCD_Write_Cmd ( CMD_SetPixel );
	for ( ucPage = 0; ucPage < dotsize; ucPage ++ ){
		for ( ucColumn = 0; ucColumn < dotsize; ucColumn ++ ){
				LCD_Write_Data ( usColor );
		}
	}
...
{	
	 ...
	 */
	int ucColumn = usC - LR, ucPage = usP - SR;
	LCD_Write_Cmd ( CMD_SetPixel );
	for(int i = 1; i< 361; i++ ){
		int startC, startP, endC, endP;
		startC = ucColumn + LR + LR*cos(i-1);
		endC = ucColumn +LR + LR*cos(i);
		startP = ucPage + SR + SR*sin(i-1);
		endP = ucPage + SR + SR*sin(i);
		LCD_DrawLine(startC, startP, endC, endP,usColor );
	}

}

void draw_name(){
	LCD_DrawLine(50, 50, 90, 50,0x0000 );
	LCD_DrawLine(90, 50, 90, 110,0x0000 );
	LCD_DrawLine(90, 110, 80, 100,0x0000 );
	LCD_DrawLine(55, 60, 85, 60,0x0000 );
	LCD_DrawLine(55, 70, 85, 70,0x0000 );
	LCD_DrawLine(55, 70, 55, 90,0x0000 );
	LCD_DrawLine(55, 90, 85, 90,0x0000 );
	LCD_DrawLine(85, 90, 85, 70,0x0000 );
}
